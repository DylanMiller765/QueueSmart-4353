    ...
    <div className="min-h-screen bg-[#f5f5f7]">
      ...
      <main className="mx-auto max-w-[1280px] px-8 py-10">
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
          ...
          <div className="lg:col-span-2" style={{ animation: "fadeUp 0.5s ease-out 0.05s both" }}>
                ...
                <div className="hidden items-center justify-center sm:flex">
                  <div className="relative flex h-32 w-32 items-center justify-center">
                    <svg className="h-full w-full -rotate-90" viewBox="0 0 120 120">
                      <circle cx="60" cy="60" r="52" fill="none" stroke="#e8e8ed" strokeWidth="6" />
                      <circle
                        cx="60" cy="60" r="52" fill="none" stroke="#0071e3" strokeWidth="6"
                        strokeLinecap="round"
                        strokeDasharray={`${0.6 * 2 * Math.PI * 52} ${2 * Math.PI * 52}`}
                      />
                    </svg>
                    <div className="absolute text-center">
                      <p className="text-[11px] font-medium uppercase tracking-wide text-muted">Wait</p>
                      <p className="text-[20px] font-semibold text-foreground">{mockCurrentQueue.estimatedWaitTime}m</p>
                    </div>
                  </div>
                </div>
              </div>
          ...
          {/* Right column */}
          <div className="flex flex-col gap-6" style={{ animation: "fadeUp 0.5s ease-out 0.1s both" }}>
            {/* Services */}
                  ...
                  >
                    <div>
                      ...
                      <p className="mt-0.5 text-[12px] text-muted">
                        {service.currentQueueLength} in queue &middot; ~{service.expectedDuration} min
                      </p>
                    </div>
                    <ChevronRight size={15} strokeWidth={1.5} className="text-[#d2d2d7] transition-transform group-hover:translate-x-0.5" />
                  </Link>
                ))}
              </div>
            </div>

            {/* Recent Activity — now using real notifications */}
                    ...
                    <div key={n.id} className="flex items-start gap-2.5">
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-baseline justify-between gap-3">
                          <p className={`text-[13px] ${!n.read ? "font-medium text-foreground" : "text-foreground/60"}`}>
                            {n.title}
                          </p>
                          <p className="shrink-0 text-[11px] text-muted/50">{n.time}</p>
                        </div>
                        <p className="mt-0.5 text-[12px] leading-relaxed text-muted">{n.message}</p>
                      </div>
                    </div>
                  ))
                )}
